        ...
        <tbody>
          {operationData && operationData.length > 0 ? (
            operationData.map((op) => {
              ...
              const alerts = Array.isArray((op as any).alerts)
                ? (op as any).alerts
                    .map((a: any) => (typeof a === 'object' && a?.message ? a.message : ''))
                    .filter(Boolean)
                    .join(', ')
                : 'None';
              return (
                <tr key={op.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <td style={{ padding: '12px 16px', fontSize: '14px', color: '#333' }}>{alerts || 'None'}</td>
